...
out_dir <- "/home/rstudio/isilon/private/projects/HumanMouseUBC/CBintegrationPaper/humanDEG"
...
# markers common to Aldinger and Sepp dataset UBC PAX6+ EOMES+ clusters 
cMarkerFile <- "/home/rstudio/isilon/private/projects/HumanMouseUBC/CBintegrationPaper/EachHumanDatasetOnly/260307/common_upregulated_genes_top_ClusterPairs.tsv"
...
cat(sprintf("Number of common markers: %s\n", nrow(common)))
...
cat("Adding genes for annotation\n")
anno_genes <- map(
  .x = feature_ranking,
  .f = \(df) {slice_min(df, order_by = rank, n = 3)}
) %>%
  unlist() %>%
  unname()
anno_genes <- union(union(anno_genes, unique(common$gene)),"MKI67")
ha <- rowAnnotation(
