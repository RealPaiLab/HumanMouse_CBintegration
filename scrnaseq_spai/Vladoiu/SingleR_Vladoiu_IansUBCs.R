...
outRoot <- "/home/rstudio/isilon/private/projects/HumanMouseUBC/SingleR_UBCclusters_Vladoiu_IanUBC"
...
singleRFile <- "" #sprintf("%s/251023/Vladoiu_singleRPred_251023.RData",outRoot)
...
if (file.exists(singleRFile)){
    ...
    cat(sprintf("Keeping genes in common: RL has %i, tumour has %i, intersecting %i\n",
        length(rl_genes), length(tumour_genes), length(intersecting_genes)))
    ...
    keep_idx <- which(!rl_srat[[]]$RLwithUBC %in% c("UBC"))
    rl_srat <- subset(rl_srat, cells = Cells(rl_srat)[keep_idx])    

    # make a dotplot of marker genes
    markerGenes <- c("WLS","SOX2","MKI67","PAX6","RBFOX3",
    "EOMES","EYS","BRCA1")
    p <- DotPlot(rl_srat, features = markerGenes, group.by="RLwithUBC") +
        RotatedAxis() +
        ggtitle("RL dev data with UBC labels integrated")
    ggsave(p,file=sprintf("%s/RLdev_with_UBC_labels_dotplot.pdf",outDir),
        width=10,height=8)

    cat("Table of new RLwithUBC labels\n")
    print(table(rl_srat[[]]$RLwithUBC,useNA="always"))

    rl_srat$RLwithUBC <- sub("UBCUBC_","UBC",rl_srat$RLwithUBC)

    #now plot dimplot again
    p <- DimPlot(rl_srat, group.by="RLwithUBC", label=TRUE) +
        ggtitle("RL dev data with UBC labels integrated")
    ggsave(p,file=sprintf("%s/RLdev_with_UBC_labels.pdf",outDir),
        width=10,height=8)
...
cat("about to run test for different quantiles\n")
browser()

for (qtl in c(0.5, 0.7, 0.9)){
    cat(sprintf("Running SingleR with quantile %.2f\n",qtl))
     t0 <- Sys.time()
    singleRPred <- classifySingleR(
      test = tumour_sce,
      trained = trained_model,
    ...
    # for each tumour subplot, plot the boxplot of scores for each SingleR label
    for (k in unique(merged$subtype)){
        x <- merged[which(merged$subtype == k),]
        scoreMat <- as.data.frame(x[,grep("score.",colnames(x))])
        scoreMat$CellID <- x$CellID
        scoreMat_melt <- reshape2::melt(scoreMat, id.vars = "CellID",
            variable.name = "SingleR_label", value.name = "Score")
        scoreMat_melt$SingleR_label <- sub("score.","",scoreMat_melt$SingleR_label)
        p <- ggplot(scoreMat_melt, aes(x=SingleR_label, y=Score)) +
            geom_boxplot() +
            theme_minimal(base_size=12) +
            ggtitle(sprintf("QTL %.2f SingleR scores for subtype %s",qtl,k)) +
            xlab("SingleR label") +
            ylab("Score") +
            theme(axis.text.x = element_text(angle = 45, hjust = 1))

        ggsave(p,file=sprintf("%s/Qtl_%.2f_SingleR_scores_boxplot_subtype_%s.pdf",outDir,qtl,k),
            width=10,height=8)
    }
}
}
...
# dotplot of MB marker genes 
cat("Plot dotplot of MB marker genes\n")
markerGenes <- c("OTX2","CRX","NRL", "MYC","MYCN","BRCA1",
    "GLI2","PAX6","EOMES","PTCH1","SMO","MKI67")
p <- DotPlot(tumour_srat, features = markerGenes, group.by="subtype") +
    RotatedAxis() +
    ggtitle("MB subtype marker genes")
ggsave(p,file=sprintf("%s/MB_subtype_marker_genes_dotplot.pdf",outDir),
    width=10,height=8)
