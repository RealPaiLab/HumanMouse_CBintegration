...
outRoot <- "/home/rstudio/isilon/private/projects/HumanMouseUBC/SingleR_UBCclusters_Vladoiu_LiamRL"

devFile <- "/home/rstudio/isilon/private/icheong/CBL_scRNAseq/data/human/Aldinger/glutamatergic_dev_Liam.RDS"
...
tryCatch({
    ...
    t0 <- Sys.time()
    tumour_mat <- NormalizeData(tumour_srat, assay = "RNA")
    print(Sys.time()-t0)
...
if (file.exists(singleRFile)){
    ...
    cat("RL labels\n")
    RL_labels <- as.data.frame(as.character(rl_srat[[]]$new_cell_type))
    ...
    trained_model <- trainSingleR(
      rl_sce, 
      labels=rl_sce$new_cell_type,#rl_sce$RLwithUBC, 
      de.method = "wilcox",
