...
mb_srat <-  "/home/rstudio/isilon/private/icheong/CBL_scRNAseq/results/tumour/Vladoiu/20250211/mb_mnn.qs"

rl_srat <- "/home/rstudio/isilon/private/projects/HumanMouseUBC/results/integrated_HsFullCB/20241031/cca/assignClusterIdentity/cca_RLlineage_only_241107.qs"

ubc_srat <- "/home/rstudio/isilon/private/projects/HumanMouseUBC/results/integrated_HsFullCB/20241031/cca/RLlineage_only/241107/UBC_withClusterAssignments_241107.qs"

num_threads <-  4
out_dir <-  "/home/rstudio/isilon/private/projects/HumanMouseUBC/tumour_projection/Vladoiu_SingleR/"
...
logFile <- sprintf("%s/SingleR_%s.log", out_dir, dt)
...
cat("***Loading MB tumour Seurat object***\n")
t0 <- Sys.time()
mb_srat <- qs::qread(mb_srat)
mb_mat <- get_norm_expr(mb_srat, rerun_norm = TRUE)

cat(Sys.time() - t0,"\n")
# print number of cells, number of genes
cat(sprintf("Number of cells: %s\nNumber of genes: %s\n",ncol(mb_mat),
nrow(mb_mat)))

cat("***Loading RL lineage Seurat object***\n")
t0 <- Sys.time()
rl_srat <- qs::qread(rl_srat)
...
cat("***Loading UBC Seurat object***\n")
t0 <- Sys.time()
ubc_srat <- qs::qread(ubc_srat)
...
subdir <- file.path(out_dir, "full_RL_lineage")
...
rl_labels <- rl_srat[[]][colnames(rl_mat), "curtype"]
...
subdir <- file.path(out_dir, "full_RL_then_UBC")
if (!dir.exists(subdir)) {
  dir.create(subdir, recursive = TRUE)
}

# get reference annotations
ubc_labels <- ubc_srat[[]][colnames(ubc_mat), "seurat_clusters"]
...
subdir <- file.path(out_dir, "merged_RL_with_UBC")
if (!dir.exists(subdir)) {
  dir.create(subdir, recursive = TRUE)
}
...
# annotations
rl_lineage <- rl_srat[[]][, "curtype"] %>%
  as.character() %>%
  setNames(nm = rownames(rl_srat[[]]))
ubc_clust <- ubc_srat[[]][, "seurat_clusters"] %>% 
  paste0("UBC_", .) %>%
...
saveRDS(
  ...
  file = file.path(
    out_dir,
    paste0(tools::file_path_sans_ext(basename(mb_srat[1])), "_metadata.rds")
  )
)
...
print(sessionInfo())
  ...
  message("Error: ", conditionMessage(ex))
  print(ex)
}, finally={
  sink(type = "message")
  close(logFileConn)
