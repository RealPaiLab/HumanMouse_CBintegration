...
outDir <- "/home/rstudio/isilon/private/projects/HumanMouseUBC/UBC1_marker_analysis/Integrated_RL_Hsonly/DEG_nofiltered241029/"
setFile <- "/home/rstudio/isilon/private/llau/results/integrated/20240618/rl_cca.qs"
normFile <- "/home/rstudio/isilon/private/projects/HumanMouseUBC/UBC1_marker_analysis/Integrated_RL_Hsonly/rl_cca_HumanOnly_NormSCTdone_241029.qs"
...
if (file.exists(normFile)) {
    srat <- qs::qread(normFile) # human only > normalizedata > sctransform run
} else {
...
message("Subsetting for UBC")
srat_ubc <- subset(srat, subset = broad_cell_type=="UBC")
srat_ubc <- ScaleData(srat_ubc,assay="SCT",features=rownames(srat_ubc))

message("should you be running SCTransform?")
browser()
...
dset <- srat_ubc@meta.data[,c("broad_w_ubc_subtypes","dataset_name")]
...
ages <- as.matrix(table(srat_ubc@meta.data[,c("broad_w_ubc_subtypes","age")]))
ages <- t(apply(ages,1,function(x) x/sum(x)))
sex <- as.matrix(table(srat_ubc@meta.data[,c("broad_w_ubc_subtypes","sex")]))
