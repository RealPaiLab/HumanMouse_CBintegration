...
if (interactive()) {
  ...
  arg_list <- parser$parse_args(c(
    "--srat_file", "/.mounts/labs/pailab/private/projects/HumanMouseUBC/UBCclusters/fromQuang/UBC.Harmony.RDS",
    "--cluster", "SCT_snn_res.0.5",
    "--out_dir", "/.mounts/labs/pailab/private/projects/HumanMouseUBC/integrated_human_ubc/CytoTRACE",
    "--num_threads", 4
  ))
} else {
...
make_violin <- function(
  ...
  walk(
    ...
    .f = \(group_by) {
      ...
      cat(sprintf("\t saved violin for %s\n", group_by))
    }
  )
}
...
pwalk(
  ...
  .f = \(batch, subdir) {
    ...
    cat(sprintf("about to make violins for %s\n", batch))
